...
document
  ...
  .addEventListener("submit", (event) => {
    ...
    try {
      ...
      console.log(`Template '${templateName}' salvo com sucesso.`); // Log de verificação
      createTemplateModal.style.display = "none";
      welcomeScreen.style.display = "none";
...
function loadTemplate(templateName) {
  const templateData = localStorage.getItem(templateName);
  if (templateData) {
    const template = JSON.parse(templateData);
    renderColumns(template.columns);
  } else {
    console.warn(`Template '${templateName}' não encontrado no localStorage.`);
  }
}
...
// Adicionar tarefa à coluna no localStorage e na interface
...
window.onload = () => {
  ...
  Object.keys(localStorage).forEach((templateName) => {
    addTemplateOption(templateName);
  });
  console.log("Templates carregados:", Object.keys(localStorage)); // Log para ver os templates carregados
};
